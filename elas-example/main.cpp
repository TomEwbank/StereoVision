...
void process (const char* file_1,const char* file_2) {
    ...
    cout << "Time for elas processing: " << elapsed.total_microseconds()/1.0e6 << "s" << endl;
    cout << "elas FPS: " << 1/(elapsed.total_microseconds()/1.0e6) << "s" << endl;

    int nbPts = 0;
    for (int32_t i=0; i<width*height; i++) {
        if (D1_data[i]>0) ++nbPts;
    }

    FileStorage fs;
    fs.open("test_imgs/stereoMatlabCalib.yml", FileStorage::READ);
    Mat Q;
    fs["Q"] >> Q;
    std::vector<Vec3d> vin2(nbPts);
    std::vector<Vec3d> vout2(nbPts);
    int j = 0;
    for (int32_t i=0; i<width*height; i++) {
        if (D1_data[i] > 0) {
            Vec3d p(i%width, (int)(i/width), D1_data[i]);
            vin2[j] = p;
            ++j;
        }
    }
    perspectiveTransform(vin2, vout2, Q);
    Mat colorLeftImg = imread("test_imgs/left_"+pairName+"_rectified.ppm", CV_LOAD_IMAGE_COLOR);
    ofstream outputFile("pointCloudElas_"+pairName+".txt");
    std::vector<Vec3d>::iterator vout2It;
    std::vector<Vec3d>::iterator vin2It;
    for (vin2It = vin2.begin(), vout2It = vout2.begin(); vout2It < vout2.end(); vout2It++, vin2It++) {
        Vec3d point3D = *vout2It;
        Point pointInImage((*vin2It).val[0], (*vin2It).val[1]);
...
int main (int argc, char** argv) {
    ...
    if (argc==2 && !strcmp(argv[1],"demo")) {
        process("img/left_1_500_02_rectified.pgm",   "img/right_1_500_02_rectified.pgm");
